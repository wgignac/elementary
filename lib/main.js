...
// current: mstack, msrow
  tokens: {
    ...
    'msgroup': function(parentNode,childNode,options){
      ...
      var shift = parseInt(childNode.getAttribute('shift')) + parentShift;
      grandChildren =  elementaryMML.getChildren(childNode);
      for (var c = 0; c < grandChildren.length; c++){
        ...
        elementaryMML.transformChild(mtr, msrow, options);
