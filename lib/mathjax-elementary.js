...
// current: mstack, msrow
  tokens: {
    'mstack': function(parentNode,childNode) {
      ...
      for (var i=0;i<rows.length;i++) {
        ...
        for (var j = 0; j < amount; j++){
          var mtd = elementaryMML.createElement('mtd');
          if (frontOp){
            // 2017/02/07: fix Swedish example: make 0; also revise amount calclulation (drop tableHasFrontOp??)
    ...
    'msrow':  function(parentNode,childNode,rowPosition) {
      ...
      if (parentNode.tagName !== 'mtr') throw new Error ('msrow not child of mtr');
